defmodule ICalendar.CalendarTest do
  ...
  describe "events" do
    ...
    test "create_event/1 with valid data creates a event" do
      valid_attrs = %{name: "some name", metadata: "some metadata", event_date: ~D[2024-07-11], duration: 42}

      assert {:ok, %Event{} = event} = Calendar.create_event(valid_attrs)
      assert event.name == "some name"
      assert event.metadata == "some metadata"
      assert event.event_date == ~D[2024-07-11]
    ...
    test "update_event/2 with valid data updates the event" do
      event = event_fixture()
      update_attrs = %{name: "some updated name", metadata: "some updated metadata", event_date: ~D[2024-07-12], duration: 43}

      assert {:ok, %Event{} = event} = Calendar.update_event(event, update_attrs)
      assert event.name == "some updated name"
      assert event.metadata == "some updated metadata"
      assert event.event_date == ~D[2024-07-12]
