defmodule ICalendarWeb.CoreComponents do
    ...
    <div>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-600 dark:text-white">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-300 text-slate-600 dark:text-blue-600 focus:ring-0"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ICalendarWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ICalendarWeb.Gettext, "errors", msg, opts)
    end
  end
